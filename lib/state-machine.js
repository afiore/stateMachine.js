!function (context) {
  ...
  // also add the module the eventEmitter api
  ...
  _.extend(StateMachine.prototype, EventEmitter.prototype);
   ...
   */
   ...
   StateMachine.prototype._implementInstanceMethod =  function (method, fromTo, onTransition) {
    ...
    this[method] = _.bind(function () {
      ...
      //run the onExit hook
      if (onExit) {
        callbackMessage = onExit.call(this);
      }
      ...
      //run the onExit hook
      if (onEnter) {
        callbackMessage = onEnter.call(this);
      }
  ...
  /**
   * Wrapper function for firing the event. 
   * It actually fires two events: a specific one (e.g. 'state:closed') and a generic 'state-change' event
   *
   * enterState - the entering state
   * data       - the data to be sent along with the event
   *
   */

  StateMachine.prototype._doEmit =  function (enterState, data) {
    this.emit('state:'+enterState, data);
    this.emit('state-change', enterState, data);
  };
  ...
  *
  */
  StateMachine.prototype.addState = function (state, onEnter, onExit) {
   ...
   *
   */
  StateMachine.prototype.addTransition = function (transition, fromTo, onTransition) {
   ...
   *
   */
   StateMachine.prototype._getOnExit = function (state) {
   ...
   *
   */
   StateMachine.prototype._getOnEnter = function (state) {
   ...
   *
   */
   StateMachine.prototype._getStateHook = function (state, stateType) {
    ...
    * returns nothing
    */
    StateMachine.prototype._validateTransitionStates = function (transition, fromTo) {
      var fromToList = [],
   ...
   *
   */
   StateMachine.prototype.initialize = function(specFunction) {
